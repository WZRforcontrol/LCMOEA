...
%--------------------------------------------------------------------------
    ...
    FrontNo = NDSort(Population.objs,Population.cons,1); % 对种群进行约束非支配排序，只关注第一层非支配解
...
function [INDEX,DIS] = Association(Population,V,k)
    ...
    PopObj    = (Population.objs - repmat(zmin,N,1))./(repmat(zmax-zmin,N,1));% 优劣解距法
    % 为每个参考向量关联 k 个候选解
    normP  = sqrt(sum(PopObj.^2,2));
    Cosine = 1 - pdist2(PopObj,V,'cosine');%计算种群目标值与参考向量的余弦相似度(1-余弦距离)矩阵
