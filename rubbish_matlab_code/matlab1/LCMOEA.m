function pareto_front =  LCMOEA(probl, varargin)
%% LCMOEA - 用于约束多目标优化的可学习进化算法
% Author: Z.R.Wang
...
addRequired(p, 'probl');
addParameter(p, 'N', 50);
addParameter(p, 'FE_max', 1000);
addParameter(p, 'hidden_sizes', 10);
addParameter(p, 'num_epochs', 100);
addParameter(p, 'learning_rate', 0.001);
addParameter(p, 'model_dir', 'models');
addParameter(p, 'is_plot', false);
addParameter(p, 'image_dir', 'images');
addParameter(p, 'video_dir', 'video');
addParameter(p, 'xlim', []);
addParameter(p, 'ylim', []);
parse(p, probl, varargin{:});
probl = p.Results.probl;
N = p.Results.N;
FE_max = p.Results.FE_max;
hidden_sizes = p.Results.hidden_sizes;
num_epochs = p.Results.num_epochs;
learning_rate = p.Results.learning_rate;
model_dir = p.Results.model_dir;
is_plot = p.Results.is_plot;
image_dir = p.Results.image_dir;
video_dir = p.Results.video_dir;
xlim = p.Results.xlim;
ylim = p.Results.ylim;
